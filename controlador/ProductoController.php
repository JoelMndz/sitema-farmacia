...
if($_POST['funcion']=='cambiar_avatar'){
  ...
  if(($_FILES['photo']['type']=='image/jpeg')||($_FILES['photo']['type']=='image/png')||($_FILES['photo']['type']=='image/gif') ||($_FILES['photo']['type']=='image/jpg')){
    ...
    if($avatar!='../img/OIP.jpeg'){
      unlink($avatar); //borrar imagen avatar que se cambió
    } 
    $json= array();
    $json[]=array(
      'ruta'=>$ruta,
    ...
    foreach ($producto->objetos as $objeto) {
      ...
      $html.="
        <tr prodId='$objeto->id_producto' prodPrecio='$objeto->precio'>
          ...
          <td>
            <input type='number' min='1' class='form-control cantidad-producto' value='$resultado->cantidad'>
          </td>
          <td class='subtotales'>
            <h5>$subtotal</h5>
          </td>
...
if($_POST['funcion']=='reporte_productos'){
  ...
  $html = '
    <header>
      <div id="logo">
        <img src="../img/ebenezer4.png" width="180" height="80">
      </div>
...
if($_POST['funcion']=='reporte_productosExcel'){
  ...
  $ruta_completa = '../Excel/'. $nombre_archivo;
